...
const CartItemImage = ({ image, isDraft }) => {
  if (!image) {
    return <div className="shopping-cart-section__item__no-image__container"></div>;
  }
  return (
    <div className="shopping-cart-section__item__image__container">
      {(image.url && <img className="shopping-cart-section__item__image" src={`${image.url}?w=44&h=75`} alt="" />) ||
        (image.icon && (
          <Icon
            className={cn('shopping-cart-section__item__image', {
              [`shopping-cart-section__item__image--${image.color || 'black'}`]: !isDraft,
              [`shopping-cart-section__item__image--dark-grey`]: isDraft,
            })}
            icon={image.icon}
            title=""
          />
...
const ShoppingCartItem = ({
  ...
  return (
    <>
      <ShoppingCartRow
        className={cn('shopping-cart-section__item', {
          'shopping-cart-section__item__dashed': isDraft,
          [`shopping-cart-section__item__id--${id}`]: id,
        })}
        key={id}
      >
        ...
        >
          <div
            className={cn('shopping-cart-section__item__name__container', {
              'shopping-cart-section__item__name--subitem': isSubItem || indent,
            })}
          >
            <CartItemImage image={cartItem.image} isDraft={isDraft} />
            <div className="shopping-cart-section__item__name__text__container">
              {name}
              {subtitle && (
                <span
                  className="shopping-cart-section__item__name__text__subtitle"
              ...
              {!!leaseMonths && leaseMonths !== 'now' && price.upfront && (
                <span className="shopping-cart-section__item__name__text__subtitle">
                  Betales nå: {`${formatPrice(price.upfront - discountValueUpfront)}`}
                </span>
              )}
              {shouldShowPricePerUnit && (
                <span className="shopping-cart-section__item__price-per">
        ...
        <div className="shopping-cart-section__item__price__wrapper">
          <ShoppingCartCell
            className={cn('shopping-cart-section__item__price', {
              'shopping-cart-section__item__price--monthly': price.monthly,
            })}
          >
            {discountValueMonthly === 0 && discountValueUpfront === 0 ? (
              ...
              <div className="shopping-cart-section__item__price__container">
                <span className="shopping-cart-section__item__price__number shopping-cart-section__item__price--discount">
                  {getPrice(formatPrice, price, 0, 0, quantity)}
                </span>
                {_.isNumber(price.monthly) && (
                  <span className="shopping-cart-section__item__price--discount">/md.{hasGroupDiscount && '*'}</span>
                )}
                <span className="shopping-cart-section__item__price__number">
          ...
          <ShoppingCartCell className="shopping-cart-section__item__delete">
            {isRemovable && !hasPaid && (
              <div className="shopping-cart-section__item__delete-button__container">
                <button
                  className="button button--small shopping-cart-section__item__delete-button"
                  onClick={() => onRemoveItem(cartItem)}
                >
                  <div className="shopping-cart-section__item__close">x</div>
                  <div className="shopping-cart-section__item__close__text">Slett</div>
                </button>
              </div>
            )}
          </ShoppingCartCell>
