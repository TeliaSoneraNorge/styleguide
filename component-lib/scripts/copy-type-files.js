    ...
    await Promise.all(
        typeFiles.map(async file => {
            ...
            if ((await fse.exists(dst)) && !(await filesAreEqual(src, dst))) {
                console.error(
                    `ERROR: dist/${file} already exists in another shape -- aborting.`
                );
                console.log(
                    `       This can be due to tsc emitting a file with the same`
                );
                console.log(
                    `       name as a file already defined in src/${file}.`
                );
                process.exit(1);
            } else {
