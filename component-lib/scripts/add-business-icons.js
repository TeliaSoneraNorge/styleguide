...
async function run() {
  ...
  const finalIconsFolder = path.resolve(__dirname, '../assets/business-icons');
  ...
  for (const i in iconFiles) {
    ...
    try {
      ...
      fs.writeFileSync(path.resolve(finalIconsFolder, iconFileName.toLowerCase()), svg.data, { flag: 'wx' });
