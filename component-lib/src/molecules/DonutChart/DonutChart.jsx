...
const constrainMinimumValue = (value, minimum) => (value < minimum ? minimum : value);

const aggregatePreviousPercentages = (segments, index) =>
  segments
    .slice(0, index + 1)
    .reduce((prev, curr) => prev + constrainMinimumValue(curr.percent, minimumPercentageValueToBeDisplayed), 0);
...
const DonutChart = ({
  ...
  <ChartWrapper className="donut-chart" size={size} link={link}>
    <svg className="donut-chart__slice" viewBox="-1 -1 2 2" width={size} height={size}>
      ...
      />
      {!loading &&
        alignSegments(segments).map((segment, index) => (
          <ChartSegment
            key={segment.percent}
            index={index}
            color={segment.color}
