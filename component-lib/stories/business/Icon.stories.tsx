...
import { AllBusinessIcons, Icon, IconDefinition } from '../../src/business/Icon';
...
stories.add('Icon', () => (
    <div style={{ padding: '24px' }}>
        ...
        <p>
            The component <code>{'<AllBusinessIcons />'}</code> (which itself is invisible) must be present in order for
            the SVGs to be available to the <code>{'<Icon />'}</code> component.
        </p>
        ...
        <div style={{ transition: 'color 0.15s ease-in-out', color: color('Color', 'red') }}>
            <Icon icon={select('Sample icon', Object.keys(availableIcons), 'ai-robot') as IconDefinition} />
        </div>

        <h2>All icons</h2>

        <ul style={{ listStyleType: 'none', margin: '0', padding: '0' }}>
            {Object.keys(availableIcons).map((key: IconDefinition) => (
                <li key={key} style={{ padding: '8px', display: 'inline-block' }} title={key}>
...
const availableIcons: { [key in IconDefinition]: true } = {
    add: true,
    'ai-robot': true,
    alarm: true,
    alert: true,
